...
- **Cascade layers:** base `a` styles live in `@layer base` and `.frame`/`.pill` in `@layer components`, so Tailwind utilities on the same element win (`hidden`, `fixed`, `bg-*`, `no-underline`). Unlayered CSS beats every utility, so keep new component classes layered. Lock page scroll on `<html>`, never `<body>`: body overflow makes it a scroll container and breaks the sticky header (same reason body uses `overflow-x: clip`).
- **Arcade game (ANOMALY):** `src/components/AnomalyGame.astro` handles UI, input and ES/EN copy (styles are global because panels are built at runtime); `src/lib/anomaly/engine.ts` is the DOM-free simulation and `render.ts` the canvas renderer. Check balance headlessly with `node scripts/anomaly-sim.ts [runs] [skill]`.
